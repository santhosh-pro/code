...
const CustomerListPage = memo(() => {
  ...
  const [params, mergeParams, loading, data, error, , refresh] = usePaginationObservable(
    params => customerService.list(params),
  ...
  const handleEdit = useCallback((current: ICustomer) => {
    setCurrent(current);
    setFormOpened(true);
  }, []);
  ...
  const formCallback = useCallback(
    (customer?: ICustomer) => {
      setFormOpened(false);
      current ? refresh() : mergeParams({ term: customer.email });
    },
  ...
  return (
    <Fragment>
      ...
      <Card>
        <FormDialog opened={formOpened} customer={current} onComplete={formCallback} onCancel={formCancel} />
          ...
          <Table>
            ...
            <TableBody>
              ...
              {results.map(customer => (
                <ListItem key={customer.id} customer={customer} onEdit={handleEdit} onDeleteComplete={refresh} />
              ))}
            </TableBody>
          </Table>
        </TableWrapper>
